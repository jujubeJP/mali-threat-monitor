...
function renderKPIs() {
  ...
  const html = `
    ...
    <div class="kpi">
      ...
      <div class="kpi-sub">タンクローリー300台以上焼失</div>
    </div>
    <div class="kpi">
...
function renderPhaseTrack() {
  ...
  const items = [
    { key: null, label: "全期間", range: "2025-05 → 2026-06", color: "#9aa1ad", count: allCount }
  ];
...
function initMap() {
  ...
  document.getElementById('map-legend').innerHTML = `
    <div class="legend-block"><div class="legend-title">標的</div><div class="legend-row">${targetLegend}</div></div>
    <div class="legend-block"><div class="legend-title">フェーズ縁色</div><div class="legend-row">${phaseLegend}</div></div>
  `;
}
    ...
    return `
      <div class="phase-col" style="--phase-color:${ph.color}">
        ...
        <div class="phase-col-stats">
          <div><span class="ps-stat-num">${total}</span><span class="ps-stat-lab">事案</span></div>
          <div><span class="ps-stat-num">${fatal}+</span><span class="ps-stat-lab">死者</span></div>
        </div>
        <div class="phase-col-section">
          <div class="phase-col-section-label">主要標的</div>
          <ul class="ps-list">${targetRows}</ul>
        </div>
        <div class="phase-col-section">
          <div class="phase-col-section-label">主要地域</div>
          <ul class="ps-list">${regionRows}</ul>
        </div>
        <div class="phase-col-section">
          <div class="phase-col-section-label">主体</div>
